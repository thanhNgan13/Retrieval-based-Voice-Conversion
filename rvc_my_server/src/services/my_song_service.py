...
def _song_view(doc: dict) -> dict:
    view = convert_firestore_doc(doc, drop_keys=_DROP_KEYS)
...
def list_my_songs(user_id: str, limit: Optional[int], start_after: Optional[str]) -> dict:
    ...
    pagination = build_pagination_block(items, norm_limit, id_field="songId")
    return {
...
def delete_my_song(user_id: str, song_id: str) -> dict:
    ...
    return convert_firestore_doc(doc, drop_keys=_DROP_KEYS)
